...
# Add a new line here if you would like to change its default config. (E.g CHANNELS = 7.12.x
CHANNELS = 7.12.x
...
# Add a new line here if you would like to change its default config. (E.g DEFAULT_CHANNEL = 7.12.x
DEFAULT_CHANNEL = 7.12.x
...
.PHONY: docker-build
docker-build: generate-deploy ## Build docker image with the manager.
	docker build -t ${IMG} .
